...
int terminal_init() {
    ...
    if (has_colors()) {
        ...
        init_pair(4, COLOR_BLUE, COLOR_BLACK);    // Walls
...
void draw_board(board_t* board, int mode) {
    debug("Drawing board for level %s, size %d x %d\n", board->level_name, board->width, board->height);
    ...
    case DRAW_MENU:
        mvprintw(1, 0, "Level: %s | W/A/S/D move | Q quit ", board->level_name);
        break;
    }
