...
int load_level(board_t *board, int points) {
        // Se não foi definido PAC no lvl, criar Pacman por defeito
        if (board->n_pacmans == 0 || board->pacmans == NULL) {
            board->pacmans = malloc(sizeof(pacman_t));
            if (!board->pacmans) {
                perror("malloc pacman");
                return -1;
            }
            ...
            pac->alive = 1;
            pac->points = 0;
            pac->passo = 0;        // começa logo
            pac->n_moves = 0;      // controlado pelo jogador
            pac->current_move = 0;
            pac->waiting = 0;
        }
    // Set entity positions on board
    if (board->n_pacmans > 0 && board->pacmans != NULL) {
...
void unload_level(board_t * board) {
    ...
    // libertar memória alocada durante load_level / lvl_parser
    free(board->board);
    free(board->pacmans);
    free(board->ghosts);

    // repor ponteiros e contadores apenas
    board->board = NULL;
    board->pacmans = NULL;
    board->ghosts = NULL;
